import PdfFooter from "@/components/PdfFooter";
import PdfHeader from "@/components/PdfHeader";

export default function Report13({ print }: { print: boolean }) {
  return (
    <div
      style={{
        position: "relative",
        minHeight: `${print ? "100vh" : "115vh"}`,
        height: "auto",
      }}
    >
      <PdfHeader />
      <div
        className="bg-[#630003] pl-4 pt-2 pb-2 mt-4"
        style={{
          fontSize: "24px",
          fontWeight: "bold",
          color: "white",
        }}
      >
        <span>Securities Portfolio</span>
      </div>

      {/* Content */}
      <div className="p-4 flex-1">
        <h2 className="text-lg font-bold">Account Name:</h2>
        <h2 className="text-lg font-bold  mb-3">Account Code:</h2>
        <h2 className="text-lg font-bold  mb-3">Security Ending Position</h2>
        {/* PESO Equities Section */}
        <div className="mb-4">
          <h2 className="text-sm font-bold mb-3">PHP</h2>
          <div className="border border-black rounded-lg overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr>
                  <th className="px-2 py-1.5 text-left">Stack Symbol</th>
                  <th className="px-2 py-1.5 text-left">Issue</th>
                  <th className="px-2 py-1.5 text-left">Net Purchase</th>
                  <th className="px-2 py-1.5 text-right">No. of Shares</th>
                  <th className="px-2 py-1.5 text-right">Average Cost Price</th>
                  {/* <th className="px-2 py-1.5 text-right">Dividend Rate</th>
                  <th className="px-2 py-1.5 text-left">
                    Optional Redemption Date
                  </th> */}
                  {/* <th className="px-2 py-1.5 text-right">Cost Price</th> */}
                  <th className="px-2 py-1.5 text-right">Market Price</th>
                  <th className="px-2 py-1.5 text-right">Market Value</th>
                  <th className="px-2 py-1.5 text-right">
                    Unrealized Gain/Loss
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="px-2 py-1.5">
                    JFC
                    {/* PHP
                    <br />
                    Jollibee Food Corporation
                    <br />
                    Series B - Preferred Shares */}
                  </td>
                  <td className="px-2 py-1.5">
                    Jollibee Foods Corporation
                    <br />
                    Series B - Preferred Shares
                  </td>
                  {/* <td className="px-2 py-1.5">17-Mar-24</td> */}
                  <td className="px-2 py-1.5 text-right">1,000,001.00</td>
                  <td className="px-2 py-1.5 text-right">100</td>
                  {/* <td className="px-2 py-1.5 text-right">1.0000%</td>
                  <td className="px-2 py-1.5">17-Mar-24</td> */}
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right text-green-600">
                    1.00↗
                  </td>
                </tr>
                <tr>
                  <td></td>
                  <td></td>
                  <td className="font-bold text-center">TOTAL</td>
                  <td className="bg-amber-50 px-2 py-1.5 font-bold">14.00</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>

        {/* USD Equities Section */}
        <div className="mb-4">
          <h2 className="text-sm font-bold mb-3">USD</h2>
          <div className="border border-black rounded-lg overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr>
                  <th className="px-2 py-1.5 text-left">Stack Symbol</th>
                  <th className="px-2 py-1.5 text-left">Issue</th>
                  <th className="px-2 py-1.5 text-left">Net Purchase</th>
                  <th className="px-2 py-1.5 text-right">No. of Shares</th>
                  <th className="px-2 py-1.5 text-right">Average Cost Price</th>
                  {/* <th className="px-2 py-1.5 text-right">Dividend Rate</th>
                  <th className="px-2 py-1.5 text-left">
                    Optional Redemption Date
                  </th> */}
                  {/* <th className="px-2 py-1.5 text-right">Cost Price</th> */}
                  <th className="px-2 py-1.5 text-right">Market Price</th>
                  <th className="px-2 py-1.5 text-right">Market Value</th>
                  <th className="px-2 py-1.5 text-right">
                    Unrealized Gain/Loss
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="px-2 py-1.5">
                    JFC
                    {/* PHP
                    <br />
                    Jollibee Food Corporation
                    <br />
                    Series B - Preferred Shares */}
                  </td>
                  <td className="px-2 py-1.5">
                    Jollibee Foods Corporation
                    <br />
                    Series B - Preferred Shares
                  </td>
                  {/* <td className="px-2 py-1.5">17-Mar-24</td> */}
                  <td className="px-2 py-1.5 text-right">1,000,001.00</td>
                  <td className="px-2 py-1.5 text-right">100</td>
                  {/* <td className="px-2 py-1.5 text-right">1.0000%</td>
                  <td className="px-2 py-1.5">17-Mar-24</td> */}
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right">1,000.00</td>
                  <td className="px-2 py-1.5 text-right text-green-600">
                    1.00↗
                  </td>
                </tr>
                <tr>
                  <td></td>
                  <td></td>
                  <td className="font-bold text-center">TOTAL</td>
                  <td className="bg-amber-50 px-2 py-1.5 font-bold">14.00</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>

      {/* Footer */}
      <PdfFooter />
    </div>
  );
}
